...
public class Robot_Autonomo extends Subsystem {
	...
	private int stepIndexIntakeElevador;
	private int stepIndexDrive;
			...
			*/
			...
			if(gameData.charAt(0)=='L'){
				...
				intakeSteps=new AutonomousStep_IntakeElevador[]{
						...
						StepFactory_IntakeElevador.getNewGrabBox(),
						StepFactory_IntakeElevador.getMoveElevatorEncoder(30000),
						StepFactory_IntakeElevador.getNewWait(1.2), //1.2
						StepFactory_IntakeElevador.getNewThrowBox()
				};
				
				driveSteps = new AutonomousStep_Drive[]{
						StepFactory_Drive.getNewWait(1.5),
						StepFactory_Drive.getNewVectorMoveEncoders(45,1,40),  //50
						//StepFactory_Drive.getNewVectorMoveEncoders(90,1,12),
						StepFactory_Drive.getNewWait(1.4), //0.7
						StepFactory_Drive.getNewVectorMoveEncoders(0,-1,50),
						StepFactory_Drive.getNewVectorMoveEncoders(90,-1,25),  //48
						StepFactory_Drive.getNewWait(1),
	...
	public void RunAuto() {
		...
		//////Crea un tread para correr las secuencias del intake en paralelo al chassis
			if(stepIndexIntakeElevador<intakeSteps.length){
				if(!intakeSteps[stepIndexIntakeElevador].isSetUp()){
					intakeSteps[stepIndexIntakeElevador].setup();
					intakeSteps[stepIndexIntakeElevador].start();
				}
				else{
					if(!intakeSteps[stepIndexIntakeElevador].isFinished()){
						intakeSteps[stepIndexIntakeElevador].run();
					}
					else{
						intakeElevadorStop();
						stepIndexIntakeElevador++;
					}
				}
			}
			else{
			...
			if(stepIndexDrive<driveSteps.length){
				if(!driveSteps[stepIndexDrive].isSetUp()){
					driveSteps[stepIndexDrive].setup();
					driveSteps[stepIndexDrive].start();
				}
				else{
					if(!driveSteps[stepIndexDrive].isFinished()){
						driveSteps[stepIndexDrive].run();
					}
					else{
						driveStop();
						stepIndexDrive++;
